...
fit_model_sr1_ag_ord <- cfa(model_sr1, data = data_ord, 
                     missing = "pairwise",
                     verbose = T)
fit_model_sr2_p_ord <- cfa(model_sr2, data = data_ord, 
                        missing = "pairwise",
                        verbose = T)
...
summary(fit_model_sr1_ag_ord, fit.measures = T, standardized = T)
summary(fit_model_sr2_p_ord, fit.measures = T, standardized = T)

summary(fit_model_sr1_ag_cont, fit.measures = T, standardized = T)
summary(fit_model_sr2_p_cont, fit.measures = T, standardized = T)

anova(fit_model_sr1_ag_ord, fit_model_sr2_p_ord)
# Model with 2 puberty factors (adrenal & gonadal) fits the self-report data better
anova(fit_model_sr1_ag_cont, fit_model_sr2_p_cont)
# This is true regardless of whether self-report vars are treated as continous or ordinal variables

fitmeasures(fit_model_sr1_ag, 
            fit.measures = c("ecvi","mfi", "pgfi"))
fitmeasures(fit_model_sr2_p, 
            fit.measures = c("ecvi","mfi", "pgfi"))
#AG fits better than P for self-report questions model only
```
```{r}
# Define Full Model 0
fullmod_0 <- "
...
#Covariances for Self-Report Questions
PDS1 ~~ PDS2 + PDS3 + PDS4 + PDS6
PDS2 ~~ PDS3 + PDS4 + PDS6
PDS3 ~~ PDS4 + PDS6
PDS4 ~~ PDS6
PBIP1 ~~ PBIP2
...
summary(fit_fullmod_1_wlsmv, fit.measures = T, standardized = T)
