...
sum_sr1_pml <- summary(fit_sr1_pml, fit.measures = T, standardized = T)
sum_sr2_pml <- summary(fit_sr2_pml, fit.measures = T, standardized = T)
...
sum_sr3_pml <- summary(fit_sr3_pml, fit.measures = T, standardized = T)
sum_sr4_pml <- summary(fit_sr4_pml, fit.measures = T, standardized = T)
...
# Second, because no HMs above worked whatsoever, we run all four models without the latent hormone (DHEA, T, E2) variables and everything loads directly onto neuroendocrine (ADR, GON, PUB) variables. These are HMv2 (hormone models version 2)
...
# fitMeasures(fit_slm1, fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
...
# fitMeasures(fit_slm3, fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
...
sem1_extract_vars <- lavPredict(fit_sem1_pml)
sem2_extract_vars <- lavPredict(fit_sem2_pml)
sem3_extract_vars <- lavPredict(fit_sem3_pml)
sem4_extract_vars <- lavPredict(fit_sem4_pml)

#Correlation Matrices for Latent Var Estimates
sem1_cor <- cor(sem1_extract_vars)
sem2_cor <- cor(sem2_extract_vars)
sem3_cor <- cor(sem3_extract_vars)
sem4_cor <- cor(sem4_extract_vars)

# "P" bio with hair predict "P" self-report
sem1_p_plot <- plot(sem1_extract_vars[,"sr_puberty"] ~ sem1_extract_vars[,"bio_puberty"], ylab = "Extracted Latent Variable Estimate for Self-Reported Puberty", xlab = "Extracted Latent Variable Estimate for Hormone Puberty (Saliva + Hair)") + title(main = "Latent Biological Puberty (Hormones)\n predicts Latent Self-Reported Puberty") + abline(lm(sem1_extract_vars[,"sr_puberty"] ~ sem1_extract_vars[,"bio_puberty"])) + 
  text(x=1.75, y=1.75, labels = bquote(italic(R)^2 == .(format(0.4034))))

# "A" bio with hair predict "A" self-report 
sem2_a_plot <- plot(sem2_extract_vars[,"adrenal_sr"] ~ sem2_extract_vars[,"bio_adrenal"], ylab = "Extracted Latent Variable Estimate\n for Self-Reported Adrenarche", xlab = "Extracted Latent Variable Estimate\n for Hormone Adrenarche (Saliva + Hair)") + title(main = "Latent Biological Adrenarche\n (DHEA & T)\n predicts Latent Self-Reported Adrenarche") + abline(lm(sem2_extract_vars[,"adrenal_sr"] ~ sem2_extract_vars[,"bio_adrenal"])) + 
  text(x=1.75, y=1.75, labels = bquote(italic(R)^2 == .(format(0.3697))))

# "G" bio with hair predict "G" self-report 
sem2_g_plot <- plot(sem2_extract_vars[,"gonadal_sr"] ~ sem2_extract_vars[,"estr"], ylab = "Extracted Latent Variable Estimate\n for Self-Reported Gonadarche", xlab = "Extracted Latent Variable Estimate\n for Hormone Gonadarche (Saliva + Hair)") + title(main = "Latent Biological Gonadarche\n (E2)\n predicts Latent Self-Reported Gonadarche") + abline(lm(sem2_extract_vars[,"gonadal_sr"] ~ sem2_extract_vars[,"estr"])) + 
  text(x=0.2, y=1, labels = bquote(italic(R)^2 == .(format(0.3926))))

# "P" bio only saliva predict "P" self-report 
sem3_p_plot <- plot(sem3_extract_vars[,"sr_puberty"] ~ sem3_extract_vars[,"bio_puberty"], ylab = "Extracted Latent Variable Estimate for Self-Reported Puberty", xlab = "Extracted Latent Variable Estimate for Salivary Puberty") + title(main = "Latent Biological Puberty (Salivary Hormones)\n predicts Latent Self-Reported Puberty") + abline(lm(sem3_extract_vars[,"sr_puberty"] ~ sem3_extract_vars[,"bio_puberty"])) + 
  text(x=1.75, y=1.75, labels = bquote(italic(R)^2 == .(format(0.4034))))

# "A" bio predict "A" self-report without hair
sem4_a_plot <- plot(sem4_extract_vars[,"adrenal_sr"] ~ sem4_extract_vars[,"sal_adrenal"], ylab = "Extracted Latent Variable Estimate\n for Self-Reported Adrenarche", xlab = "Extracted Latent Variable Estimate\n for Salivary Adrenarche") + title(main = "Latent Biological Adrenarche\n (Salivary DHEA & T)\n predicts Latent Self-Reported Adrenarche") + abline(lm(sem4_extract_vars[,"adrenal_sr"] ~ sem4_extract_vars[,"sal_adrenal"])) + 
  text(x=1.75, y=1.75, labels = bquote(italic(R)^2 == .(format(0.3697))))

# "G" bio predict "G" self-report without hair
sem4_g_plot <- plot(sem4_extract_vars[,"gonadal_sr"] ~ sem4_extract_vars[,"sal_gonadal"], ylab = "Extracted Latent Variable Estimate\n for Self-Reported Gonadarche", xlab = "Extracted Latent Variable Estimate\n for Salivary Gonadarche") + title(main = "Latent Biological Gonadal Development\n (Salivary E2)\n predicts Latent Self-Reported Gonadarche") + abline(lm(sem4_extract_vars[,"gonadal_sr"] ~ sem4_extract_vars[,"sal_gonadal"])) + 
  text(x=0.2, y=1, labels = bquote(italic(R)^2 == .(format(0.3926))))
