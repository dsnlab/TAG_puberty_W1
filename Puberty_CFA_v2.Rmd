...
## PART 1: SELF-REPORT MODELS ONLY - NO METHOD FACTORS
```{r SRMs_1_2}

# Define SRM1 - Self-Report Only with 1 Puberty Factor. No method factors (but some with covariances)
...
# Define SRM2 - Self-Report Only with 2 Puberty Factors (Adrenal & Gonadal). No method factors (but some with covariances).
...
# Fit Self-Report CFAs (ordinal data) - WLSMV
...
# Fit Self-Report CFAs (ordinal data) - PML
...
# Model Stats & Comparison - Self-report, no method factors
...
fitmeasures(fit_model_sr1_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr1_ord_nocov_noPBIP,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr2_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr2_ord_nocov_noPBIP,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
#Nothing really... model already fits excellently (both of them...)

#Is self-reported puberty better captured by A+G or P? (No method factors)
...
## PART 2: SELF-REPORT MODELS ONLY - WITH METHOD FACTORS
```{r SRMs_3_4}

# Define SRM3 - Self-Report Only with 1 Puberty Factor. WITH method factors
...
#Methods Factors 
PDS =~ PDS1 + PDS2 + PDS3 + PDS4 + PDS6
PBIP =~ PBIP1 + PBIP2

#Set Method Factors Latent Var Covariances to Zero
PDS ~~ 0*puberty + 0*PBIP
PBIP ~~ 0*puberty 
"

model_sr3_noPDS1_nocov <- "
#Latent Neuroendocrine Systems
puberty =~ PDS2 + PDS3 + PDS4 + PDS6 + PBIP1 + PBIP2

#Methods Factors 
PDS =~ PDS2 + PDS3 + PDS4 + PDS6
PBIP =~ PBIP1 + PBIP2
...
# Define SRM4 - Self-Report Only with 2 Puberty Factors (Adrenal & Gonadal). WITH method factors
...
#Set Method Factors Latent Var Covariances to Zero
PDS ~~ 0*adrenal + 0*gonadal + 0*PBIP
PBIP ~~ 0*adrenal + 0*gonadal 
"

model_sr4_noPDS1_nocov <- "
#Latent Neuroendocrine Systems
adrenal =~ PDS2 + PDS3 + PBIP2
gonadal =~ PDS4 + PDS6 + PBIP1
...
fit_model_sr3_ord_nocov <- cfa(model_sr3_nocov, data = data_ord, 
                                  estimator = "WLSMV", 
                       missing = "pairwise", verbose = T) # Could not compute standard errors! The information matrix could not be inverted. This may be a symptom that the model is not identified.

fit_model_sr3_ord_noPDS1_nocov <- cfa(model_sr3_noPDS1_nocov, data = data_ord, 
                                   estimator = "WLSMV", 
                       missing = "pairwise", verbose = T) # as above :(


fit_model_sr4_ord_nocov <- cfa(model_sr4_nocov, data = data_ord, 
                                 estimator = "WLSMV", 
                       missing = "pairwise", verbose = T) # the optimizer warns that a solution has NOT been found!

fit_model_sr4_ord_noPDS1_nocov <- cfa(model_sr4_noPDS1_nocov, data = data_ord, 
                                  estimator = "WLSMV", 
                       missing = "pairwise", verbose = T) # the optimizer warns that a solution has NOT been found!

# Fit Self-Report CFAs (ordinal data) - PML

fit_model_sr3_pml_nocov <- cfa(model_sr3_nocov, data = data, estimator = "PML", 
                      missing = "available.cases", verbose = T,  
                     ordered = c("PDS1","PDS2","PDS3","PDS4",
                                 "PDS6","PBIP1","PBIP2")) # the optimizer warns that a solution has NOT been found!

fit_model_sr3_pml_noPDS1_nocov <- cfa(model_sr3_noPDS1_nocov, data = data, 
                                      estimator = "PML",
                      missing = "available.cases", verbose = T,  
...
fit_model_sr4_pml_nocov <- cfa(model_sr4_nocov, data = data, estimator = "PML", 
                      missing = "available.cases", verbose = T,  
                     ordered = c("PDS1","PDS2","PDS3","PDS4",
                                 "PDS6","PBIP1","PBIP2")) # the optimizer warns that a solution has NOT been found!
...
fit_model_sr3_cont_nocov <- cfa(model_sr3_nocov, data = data, 
                     missing = "pairwise",
                     verbose = T) # the optimizer warns that a solution has NOT been found!
fit_model_sr3_cont_noPDS1_nocov <- cfa(model_sr3_noPDS1_nocov, data = data, 
                     missing = "pairwise",
                     verbose = T) # Could not compute standard errors! The information matrix could not be inverted. This may be a symptom that the model is not identified.

fit_model_sr4_cont_nocov <- cfa(model_sr4_nocov, data = data, 
                        missing = "pairwise",
                        verbose = T) # the optimizer warns that a solution has NOT been found!
fit_model_sr4_cont_noPDS1_nocov <- cfa(model_sr4_noPDS1_nocov, data = data, 
                     missing = "pairwise",
                     verbose = T) # Wait for it... the optimizer warns that a solution has NOT been found!

# Model Stats & Comparison - Self-report, WITH method factors

#Should we include PDS1?
fitmeasures(fit_model_sr3_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
summary(fit_model_sr3_ord_nocov, fit.measures = T, standardized = T)

fitmeasures(fit_model_sr3_ord_noPDS1_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr3_pml_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr3_pml_noPDS1_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))



fitmeasures(fit_model_sr4_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
summary(fit_model_sr4_ord_nocov, fit.measures = T, standardized = T)

fitmeasures(fit_model_sr4_ord_noPDS1_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr4_pml_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr4_pml_noPDS1_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))



#Is self-reported puberty better captured by A+G or P? (WITH method factors)
#Model Stats, Fit Indices, and Comparisons 
#data treated as ordinal, and wlsmv estimator 
summary(fit_model_sr3_ord_nocov, fit.measures = T, standardized = T)
summary(fit_model_sr4_ord_nocov, fit.measures = T, standardized = T)
anova(fit_model_sr3_ord_nocov, fit_model_sr4_ord_nocov)
lavTestLRT(fit_model_sr3_ord_nocov, fit_model_sr4_ord_nocov)
fitmeasures(fit_model_sr3_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr4_ord_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))

# data treated as ordinal, and pml estimator
summary(fit_model_sr3_pml_nocov, fit.measures = T, standardized = T)
summary(fit_model_sr4_pml_nocov, fit.measures = T, standardized = T)
lavTestLRT(fit_model_sr3_pml_nocov, fit_model_sr4_pml_nocov)
fitmeasures(fit_model_sr3_pml_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
fitmeasures(fit_model_sr4_pml_nocov,
            fit.measures = c("cfi","ecvi","mfi", "chisq", "pvalue", "rmsea"))
...
## Step 3: Hormone Models - Saliva + Hair - No Method Factors. All have no equality Constraints (Correlated Uniqueness)
```{r HMs_1_2}
# Define HM1 - one factor, no methods factors, hair + saliva
hm1 <- "
...
#Covariances for Hair Sample - add back in if needed
...
# Define HM2 - two factors, no methods factors, hair + saliva
hm2 <- "
#Latent Saliva Variables
sal_dhea =~ dhea1 + dhea2 + dhea3 + dhea4
sal_estr =~ estr1 + estr2 + estr3 + estr4
...
# Fit HM2
fit_hm2 <- cfa(hm2, data = hormones, missing = "ML", estimator = "MLR", se = "robust.mlr") #the optimizer warns that a solution has NOT been found! (also even if hair covariances are back in)
...
## Step 4: Hormone Models - Saliva + Hair - WITH Method Factors
```{r HMs_3_4}
# Define HM3 - one factor, with method factors, saliva + hair
hm3 <- "
#Latent Saliva Variables
sal_dhea =~ dhea1 + dhea2 + dhea3 + dhea4
sal_estr =~ estr1 + estr2 + estr3 + estr4
...
"
# Fit HM3
fit_hm3 <- cfa(hm3, data = hormones, missing = "ML", estimator = "MLR", se = "robust.mlr") # the dread pirate not positive definite
...
# Define HM4 - two factor, with method factors, saliva + hair
hm4 <- "
#Latent Saliva Variables
sal_dhea =~ dhea1 + dhea2 + dhea3 + dhea4
sal_estr =~ estr1 + estr2 + estr3 + estr4
...
"
# Fit HM4
fit_hm4 <- cfa(hm4, data = hormones, missing = "ML", estimator = "MLR", se = "robust.mlr") # Could not compute standard errors! The information matrix could not be inverted. This may be a symptom that the model is not identified. 
